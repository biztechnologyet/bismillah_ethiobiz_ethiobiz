...
(function () {
  ...
  function boot(conf) {
    var isWebsitePage = document.body && (
      document.body.classList.contains('website-page') ||
      (!window.location.hash.includes('#') && document.getElementById('page-container') !== null)
    );

    // On website pages: check enable_website_animation setting
    // On desk/app pages: also check enable_desk_animation setting
    if (conf.enable_website_animation === false) {
      console.log("[EthioBiz] Ambient background disabled by settings");
      return;
    }

    // For desk pages, also respect enable_desk_animation
    if (!isWebsitePage && conf.enable_desk_animation === false) {
      console.log("[EthioBiz] Desk animation disabled by settings");
      return;
    }
